...
import { configureCloudinary, updateHtmlImagesToCloudinary, getCloudinaryUrl } from './lib/cloudinary'
import { PUBLIC_ASSET_PATH } from './data/cloudinary'
import { ERROR_CLOUD_NAME_REQUIRED, ERROR_NETLIFY_HOST_UNKNOWN, ERROR_NETLIFY_HOST_CLI_SUPPORT } from './data/errors';
...
integration.addBuildHook('onBuild', async ({ netlifyConfig, constants, inputs, utils }) => {    
  ...
  const {
    ...
    imagesPath = (CLOUDINARY_ASSET_DIRECTORIES as any).find(({ inputKey }: {inputKey: string}) => inputKey === 'imagesPath').path
  } = inputs;

  if (!host && deliveryType === 'fetch') {
    console.warn(ERROR_NETLIFY_HOST_UNKNOWN);
    console.log(ERROR_NETLIFY_HOST_CLI_SUPPORT);
    return;
  }
  ...
  try {
    (_cloudinaryAssets as any).images = await Promise.all(imagesFiles.map(async image => {
  ...
  if (deliveryType === 'upload') {
    await Promise.all(Object.keys(_cloudinaryAssets).flatMap(mediaType => {
      return (_cloudinaryAssets as any)[mediaType].map(async (asset: any) => {
...
integration.addBuildHook('onPostBuild', async ({ constants, inputs, utils }) => {
